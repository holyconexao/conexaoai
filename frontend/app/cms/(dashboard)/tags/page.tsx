...
import { Plus, Hash, MoreHorizontal } from "lucide-react";
...
export default function CmsTagsPage() {
  ...
  async function loadTags() {
    setIsLoading(true);
    try {
      const data = await cmsFetch<{results?: Tag[]} | Tag[]>("/tags/");
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium mt-1">Palavras-chave e tópicos específicos.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <form onSubmit={handleCreate} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
            <h2 className="font-bold text-slate-900">Adicionar Nova Tag</h2>
            <div>
            ...
            <button type="submit" className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
              <Plus className="w-4 h-4" /> Guardar Tag
            </button>
          </form>
        </div>
