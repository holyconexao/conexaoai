...
- **Manager:** Capacidade de gerir posts, categorias, tags e autores.
- **User:** Capacidade de criar e editar posts e media assets.
...
## 9. Hardening & Segurança (V1.1)

Recentemente, foram implementadas as seguintes medidas de segurança e boas práticas essenciais:

### 🔐 Segurança de Acesso e Dados
- **Hashing de Alta Segurança:** Configuração do Django para utilizar `Argon2` como hasher principal (mais resistente a ataques de força bruta e GPU).
- **Controle de Acesso (RBAC):** Implementação de permissões customizadas (`admin`, `manager`, `editor`) no nível do backend (DRF) para restringir ações baseadas no perfil do utilizador.
- **Proteção contra XSS:**
  - **Backend:** Sanitização rigorosa de campos HTML/Markdown no CMS usando `bleach`.
  - **Frontend:** Uso de `isomorphic-dompurify` ao renderizar conteúdo injetado.
- **Segurança de API:** Throttling (Rate Limiting) ativo para prevenir abusos e ataques automatizados.
- **Proteção CSRF/Sessão:** Cookies configurados como `Strict` e `Secure` (em produção), com autenticação JWT.

### 🧪 Testes e Qualidade
- **Suite de Testes Backend:** Configuração do `pytest` com cobertura de código (`pytest-cov`).
- **Testes de Segurança:** Implementação de testes unitários para validar hashing de passwords e fluxos de RBAC.
- **Validação de Dados:**
  - **Backend:** Serializers do DRF com validações estritas de tipos e conteúdo.
  - **Frontend:** Implementação de `react-hook-form` com `zod` para validação de formulários em tempo real (UX + Segurança).

### ⚙️ Operações (DevOps)
- **CI/CD:** Workflow do GitHub Actions configurado para executar linting, build e testes automatizados em cada push/PR.
- **Monitorização de Erros:** Integração do Sentry no backend para registo automático de exceções e ações críticas.
- **Backups:** Script automatizado para backup e rotação da base de dados (SQLite backups diários).
